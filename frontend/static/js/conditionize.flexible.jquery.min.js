                ...
                for (
                    var $section = $(this),
                        ...
                        ifNotInQuotes =
                            "(?:(?=([^\"]*\"[^\"]*\")*[^\"]*$)(?=([^']*'[^']*')*[^']*$))",
                        re = new RegExp(
                            "(#?[0-9a-z-_]*[a-z][0-9a-z-_]*)" + ifNotInQuotes,
                            "gi"
                        ),
                        match = re.exec(cond),
                        inputs = {},
                    ...
                    (name = match[1]),
                        (e =
                            "#" == name.substring(0, 1)
                                ? name
                                : "[name=" + name + "]"),
                        !$(e).length || name in inputs || (inputs[name] = e),
                        (match = re.exec(cond));
                for (name in inputs)
                    (e = inputs[name]),
                        (tmp_re = new RegExp(
                            "(" + name + ")\\b" + ifNotInQuotes,
                            "g"
                        )),
                        (cond =
                            "radio" == $(e).attr("type") ||
